...
impl FromStr for Play {
    ...
    fn from_str(input: &str) -> Result<Play, Self::Err> {
        ...
        let cubes = re
            .captures_iter(input)
            .map(|c| c.extract())
            .filter_map(|(_, [number, color])| {
                Some((number.parse().ok()?, Color::from_str(color).ok()?))
            })
...
impl Game {
    ...
    fn minimum_power(&self) -> usize {
        let cubes = self.plays.iter().flat_map(|play| play.cubes.iter());
        let mut max: HashMap<Color, usize> = HashMap::new();

        for cube in cubes {
            match max.get(&cube.color) {
                Some(current_max) => {
                    if cube.amount > *current_max {
                        max.insert(cube.color, cube.amount);
                    }
                },
                None => { max.insert(cube.color, cube.amount); },
            }
        }

        max.into_values().reduce(|acc, value| acc * value).unwrap_or(0)
    }
}
...
impl FromStr for Game {
    ...
    fn from_str(input: &str) -> Result<Game, Self::Err> {
        ...
        let plays = plays
            .split(";")
            .filter_map(|str| Play::from_str(str).ok())
            .collect();
